...
var deleteFromImcompletedList = function deleteFromImcompletedList(target) {
  document.getElementById("imcompleted-list").removeChild(target);
};

// 未完了リストを作製する関数
var createImcompletedList = function createImcompletedList(inputText) {
  ...
  // ボタンタグの追加処理(完了)
  var compltetedButton = document.createElement("button");
  compltetedButton.innerText = "完了";

  // 完了ボタンが押された時の処理
  compltetedButton.addEventListener("click", function () {
    // divタグの削除
    deleteFromImcompletedList(compltetedButton.parentNode); //親のdivタグを渡す

    // ulタグ内のテキスト抽出
    var addTargetParent = compltetedButton.parentNode;
    var text = addTargetParent.firstElementChild.innerText;
    // divタグ内を初期化
